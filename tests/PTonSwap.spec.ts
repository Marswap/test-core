...
describe('Router', () => {
    ...
    it('should fulfill the proxy_swap request', async () => {
        ...
        const proxySwapResult = await router.sendProxySwap(user.getSender(), {
            jettonAmount: toNano('1'),
            walletTokenBAddress: routerJettonBWalletAddress,
        ...
        const jettonSwapResult = await userJettonBWallet.sendTransfer(user.getSender(), {
            value: toNano('0.5'),
            toAddress: router.address,
            fwdAmount: toNano('0.4'),
            fromAddress: user.address,
        ...
        expect(jettonSwapResult.transactions).toHaveTransaction({
            from: userJettonBWalletAddress,
            to: routerJettonBWalletAddress,
            success: true,
            outMessagesCount: 2
        })
